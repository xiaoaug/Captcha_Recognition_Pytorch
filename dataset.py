...
class CaptchaDataset(Dataset):
    def __init__(self, path: str, transform=None, label_transform=None):
        ...
        self.image_paths = [
            os.path.join(path, image_name) for image_name in os.listdir(path)
        ]
    ...
    def __getitem__(self, idx):
        ...
        assert len(image_name) == create_captcha.CAPTCHA_NUM_LEN  # 不符合要求的 label 则抛异常
        ...
        label = []
        for i in image_name:
            tmp = [0] * create_captcha.ALL_CHAR_LEN
            tmp[create_captcha.ALL_CHAR.find(i)] = 1  # 将索引对应的值置 1
            label += tmp
...
def get_train_data_loader() -> torch.utils.data.DataLoader:
    ...
    train_transform = transforms.Compose(
        [
            transforms.Resize(
                (create_captcha.IMAGE_HEIGHT, create_captcha.IMAGE_WIDTH)
            ),
            transforms.ToTensor(),
    ...
    train_dataset = CaptchaDataset(
        create_captcha.TRAIN_DATASETS_PATH, transform=train_transform
    )
...
def get_test_data_loader() -> torch.utils.data.DataLoader:
    ...
    test_dataset = CaptchaDataset(
        create_captcha.TEST_DATASETS_PATH, transform=test_transform
    )
    test_dataloader = DataLoader(
        dataset=test_dataset,
        batch_size=train.BATCH_SIZE,  # 每次测试的样本数
...
def get_predict_data_loader() -> torch.utils.data.DataLoader:
    ...
    pred_dataset = CaptchaDataset(
        create_captcha.PREDICT_DATASETS_PATH, transform=pred_transform
    )
